...
class Network(nn.Module):
    def __init__(self, resnet, feature_dim, class_num, n_participant, alpha):
        ...
        # 3. Participant-specific head (PS)
        self.ps_predictor = nn.Sequential(

            nn.Linear(self.resnet.rep_dim, self.resnet.rep_dim),
            nn.ReLU(),
            nn.Linear(self.resnet.rep_dim, self.n_participant),
            nn.Softmax(dim=1)
        ) # size batch_size x n_participant


    @staticmethod
    def stickbreak(pi, alpha):
        """Function to apply stick-breaking prior to output probabilities of cluster head"""
        ...
        # Calculate the mean of log_prob only with non-zero values
    ...
    def forward(self, x_i, x_j):
        ...
        # participant-specific head
        p_i = self.ps_predictor(h_i)
        p_j = self.ps_predictor(h_j)


        return z_i, z_j, c_i, c_j, s_i, s_j, p_i, p_j
    ...
    def forward_participant(self, x):
        h = self.resnet(x)
        g = self.ps_predictor(h)
